...
- (void)setStatus:(AudioPlayStatus)status
{
    ...
    LogInfo(@"== AudioPlayStatus: %ld", status);
}
...
//    LogInfo(@"== current:%lf total:%lf 进度:%f", current, total, progress);
}
...
//    LogInfo(@"== start:%lf  durrantion:%lf  共缓冲%.2f 进度:%f", CMTimeGetSeconds(timeRange.start), CMTimeGetSeconds(timeRange.duration), totalBuffer, progress);
}
...
//            LogInfo(@"==KVO: 播放速率:%lf", player.rate);
        }
    }
